...
console.log(initialArray.filter((n) => {
    // Even Number
    if (n % 2 == 0) {
        return false;
    }
...
const filteredNames = filteredUsers2.map((user) => `${user.firstName} ${user.lastName}`);
console.log(filteredNames);

const filteredNames1 = users.filter((user) => user.age > 57).map((user) => `${user.firstName} ${user.lastName}`);
